...
extracted_feature = np.zeros((number, 21), dtype='int16')

# Extracte key features of all samples
for i in range(number):
    ...
    extracted_feature[i] = [mean_ax, median_ax,std_ax, diff_ax, std_ay, diff_ay, median_az, std_az, diff_az,
                            std_gz, diff_gz, skew_gz, kurt_gz,correlation_mean_a,correlation_xz_g,
...
# Save selected key features in './feature_extracted.txt', which will be used for classification.
x = np.real(extracted_feature)
save(x,'./feature_extracted.xlsx')
